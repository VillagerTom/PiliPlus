...
apply_patches() {
    ...
    while IFS= read -r id; do
        ...
        url=$(jq -r ".patches.\"$id\".url // empty" "$config")
        cd "$PROJECT_ROOT"
        if git apply "$patch_dir/$file" 2>/dev/null; then
            echo "$file applied to project${url:+ ($url)}"
        fi
    done < <(jq -r ".platforms.\"$platform\".project_patches[] // empty" "$config")
    ...
    if [[ "$(jq -r ".platforms.\"$platform\".engine_override // false" "$config")" == "true" ]]; then
    ...
    while IFS= read -r name; do
        ...
        url=$(jq -r ".commits.\"$name\".url // empty" "$config")
        if git stash 2>/dev/null; then
            if git cherry-pick "$hash" --no-edit 2>/dev/null; then
                git reset --soft HEAD~1
                echo "cherry-pick ${hash:0:9} ($name) applied${url:+ ($url)}"
            fi
            git stash pop 2>/dev/null || true
        fi
    done < <(jq -r ".platforms.\"$platform\".picks[] // empty" "$config")
    ...
    while IFS= read -r name; do
        ...
        if git stash 2>/dev/null; then
            if git revert "$hash" --no-edit 2>/dev/null; then
                git reset --soft HEAD~1
                echo "revert ${hash:0:9} ($name) applied${url:+ ($url)}"
            fi
            git stash pop 2>/dev/null || true
        fi
    done < <(jq -r ".platforms.\"$platform\".reverts[] // empty" "$config")
    ...
    while IFS= read -r id; do
        ...
        url=$(jq -r ".patches.\"$id\".url // empty" "$config")
        if git apply "$patch_dir/$file" 2>/dev/null; then
            echo "$file applied${url:+ ($url)}"
        fi
    done < <(jq -r '.common_patches[]' "$config"; jq -r ".platforms.\"$platform\".patches[] // empty" "$config")

    # Android post-processing
    if [[ "$(jq -r ".platforms.\"$platform\".engine_override // false" "$config")" == "true" ]]; then
        local engine_hash
        engine_hash=$(jq -r '.engine_version' "$config")
        echo "$engine_hash" > "$flutter_root/bin/internal/engine.version"
...
gen_build_info() {
    local platform=""
    local localdev=false
    while [[ $# -gt 0 ]]; do
        case "$1" in
            --localdev) localdev=true; shift ;;
            --platform) platform="$2"; shift 2 ;;
            *) echo "Unknown option: $1" >&2; exit 1 ;;
        esac
    done
    ...
    if grep -qE '^\s*version:\s*([0-9.]+)' pubspec.yaml; then
        version_name=$(grep -E '^\s*version:\s*([0-9.]+)' pubspec.yaml | head -n1 | sed -E 's/^\s*version:\s*([0-9.]+).*/\1/')
